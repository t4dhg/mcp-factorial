...
- **Registro horario (attendance time entry).** `factorial_attendance` grows from five to eleven actions. `clock_in`, `clock_out` and `status` for live clocking; `gaps` to list the workdays in a range where the contract expects more hours than were tracked; `log_range` to apply a daily pattern of segments to every workable day in a range; `log_days` to write an explicit list of days, for migrations from another platform. Workable days are decided from `worked_times.day_type` (the company's holiday calendar surfaces there; `estimated_times` reports full expected minutes on bank holidays) and `estimated_times.expected_minutes`. Approved leave is read from `timeoff/leaves` and skipped unless `skip_leave: false`; half-day leave days are left out of `log_range` and named. Future dates are refused. Conflicts are detected per segment by interval overlap against existing shifts (including open ones), so a planned 09:00-14:00 is skipped on a day with a live 09:15-13:15 and an afternoon segment still writes. Records are sent with `source: "api"`.
- **Confirmation tokens for attendance writes.** A new target-identity gate (`src/attendance/gate.ts`, on the previously unused `confirmation.ts` token manager) protects every attendance write aimed at someone other than `FACTORIAL_EMPLOYEE_ID`, every attendance write when that variable is unset, and every bulk write regardless of target. The first call returns a preview naming the person, dates and totals plus a token bound to a fingerprint of the plan; the confirming call re-plans and refuses if the fingerprint changed. A structural test requires every write arm in the attendance tool to consult the gate, and the README's list of token-gated operations must match the code.
- **`FACTORIAL_EMPLOYEE_ID`** (optional): the default employee for attendance actions and the identity whose own writes need no token. Validated as a positive integer; resolved to a name on first use and printed by `status`.
- New endpoints: `attendance/open_shifts`, `attendance/estimated_times`, `attendance/worked_times`, `attendance/shifts/clock_in`, `attendance/shifts/clock_out`. New schemas `OpenShiftSchema`, `EstimatedTimeSchema`, `WorkedTimeSchema`, with fixtures captured from API 2026-07-01.
- Query parameters may now be arrays; they are sent Rails-style (`employee_ids[]=1&employee_ids[]=2`).

### Changed

- **BREAKING: `ShiftSchema` and `CreateShiftInputSchema` describe the real API.** The shift record has `date`, `reference_date`, `minutes`, `clock_in_with_seconds`, `observations`, `location_type`, `half_day`, `workable`, `workplace_id`, `in_source`, `out_source` and geolocation fields; `worked_hours`, `break_minutes`, `location` and `notes` never existed. `clock_in` and `clock_out` are `HH:MM` company local strings, not ISO 8601. Creating a shift requires `date`; `clock_in` and `clock_out` accept `HH:MM` or ISO 8601 with an explicit offset. Attendance and leave responses are now validated at runtime.
- **BREAKING: `factorial_attendance list` filters.** `employee_ids`, `start_on`, `end_on`, `ids`, `updated_at`, `workable` and `half_day` replace `employee_id`, `clock_in_gte` and `clock_in_lte`, which the API silently ignored (the tool used to return every shift in the company). A list without `start_on` and `end_on`, `ids` or `updated_at` is refused. `page` and `limit` are applied client-side because the endpoint ignores them and returns everything.
- **BREAKING: `LeaveSchema` describes the real API.** `approved: boolean | null` replaces the `status` enum that never existed; `half_day` is `null`, `beggining_of_day` or `end_of_day` (Factorial's spelling); `employee_full_name`, `leave_type_name`, `company_id`, `reason`, `start_time` and `hours_amount_in_cents` added. `listLeaves` sends `employee_ids[]`, `from` and `to`, the filters the endpoint honours, instead of `employee_id`, `status`, `start_on_gte` and `start_on_lte`, which it ignored. The `list_leaves` summary reports `approved`, `half_day` and `deleted_at`.
- `delete_shift` now also fetches the shift first and, when it belongs to someone other than the configured identity, requires a confirmation token in addition to `confirm: true`.
- `write-safety.ts` gains `clock_in`, `clock_out` (low risk) and `backfill_shifts` (high risk) so the risk table stays complete.
...
- The `half_day` values accepted by leave **create** and **update** (`all_day`, `start`, `finish` in `CreateLeaveInputSchema`) were never verified against the API, which serialises `beggining_of_day` and `end_of_day` on read. Unchanged here because verifying it needs a write.
...
- Documented the Codecov integration
